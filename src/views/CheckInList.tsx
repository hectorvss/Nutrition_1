...
export default function CheckInList({ onViewHistory }: CheckInListProps) {
  ...
  const checkInClients = clients.map((c) => ({
    id: c.id as any, // ID might be string in DB but number here historically, casting safely for UI
    name: c.name,
    status: null as string | null,
    statusColor: '',
    adherence: c.progress || 100,
  ...
  const filteredClients = checkInClients.filter(client => {
    const matchesSearch = client.name.toLowerCase().includes(searchQuery.toLowerCase());
    if (filter === 'All') return matchesSearch;
    if (filter === 'Unreviewed') return matchesSearch && client.unreviewed;
    if (filter === 'Completed') return matchesSearch && !client.unreviewed;
            ...
            <div className="p-5 flex flex-col md:flex-row items-center gap-6">
              <div className="flex items-center gap-4 flex-1 w-full">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    {client.status && (
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border uppercase tracking-wider ${client.statusColor}`}>
                        {client.status}
                      </span>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-8 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 pt-4 md:pt-0">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-center">
                    <p className="text-lg font-bold text-slate-900">{client.calories}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Avg Cal</p>
                  </div>
