          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="flex items-center justify-between">
                  <button
                    ...
                    className="flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white font-bold transition-all"
                  >
                      <ChevronRight className="w-5 h-5 rotate-180" />
                      {t('back_to_monitoring')}
                  </button>
                  <span className="px-4 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-900/40">
                      {t('completed_onboarding')}
                  </span>
              </div>

              <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
                  <div className="flex items-center gap-6 mb-10 border-b border-slate-50 dark:border-slate-800 pb-8">
                      <div className="w-20 h-20 rounded-3xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-3xl text-slate-400 overflow-hidden shadow-lg border-4 border-white dark:border-slate-700">
                      ...
                      <div>
                          <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{selectedSubmission.full_name}</h2>
                  ...
                  <div className="space-y-12">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          {Object.entries(selectedSubmission.lastSubmission.answers_json || {}).map(([key, value]: [string, any]) => (
                              <div key={key} className="group p-6 rounded-2xl bg-slate-50/50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 hover:bg-white dark:hover:bg-slate-800 hover:border-emerald-100 dark:hover:border-emerald-900/40 hover:shadow-md hover:shadow-emerald-500/5 transition-all">
                                  <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3 group-hover:text-emerald-500 transition-colors">{labelMap[key] || key.replace(/_/g, ' ')}</p>
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-20 text-center border-dashed">
            <div className="w-20 h-20 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto mb-6">
                <Users className="w-10 h-10 text-slate-300" />
            </div>
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">{t('no_client_activity_found')}</h3>
            <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-sm mx-auto">{t('onboarding_monitoring_empty_desc')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSubmissions.map((s) => (
            <div key={s.id} className="group bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 flex flex-col shadow-sm hover:shadow-xl hover:border-emerald-200 dark:hover:border-emerald-900/40 transition-all transform hover:-translate-y-1">
              <div className="flex justify-between items-start mb-6">
                <div className="w-14 h-14 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-xl text-slate-400 overflow-hidden shadow-inner">
                    {s.avatar_url ? (
                ...
                <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                    s.status === 'completed' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/40' :
                    s.status === 'pending' ? 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-900/40' :
              ...
              <div className="mb-6">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white group-hover:text-emerald-600 transition-colors">{s.full_name}</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 font-medium truncate">{s.email}</p>
              </div>
                ...
                {s.status === 'completed' ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between text-[11px] font-bold">
                        ...
                        <span className="text-slate-900 dark:text-white">{new Date(s.lastSubmission.submitted_at).toLocaleDateString(locale)}</span>
                    </div>
                    <button 
                   ...
                   <div className="space-y-3">
                    <div className="flex items-center justify-between text-[11px] font-bold">
                        ...
                        <span className="text-slate-900 dark:text-white">{new Date(s.activeAssignment.assigned_at).toLocaleDateString(locale)}</span>
                    </div>
                    <div className="w-full py-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-slate-400 text-xs font-bold flex items-center justify-center gap-2 cursor-default">
                        <Clock className="w-4 h-4" />
                        {t('awaiting_response')}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 text-center py-2">
                    ...
                    <button
                         ...
                         className="w-full py-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-xs font-bold flex items-center justify-center gap-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:border-emerald-200 dark:hover:border-emerald-900/40 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all disabled:opacity-50"
                    >
